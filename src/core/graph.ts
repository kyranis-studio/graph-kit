...
export class GraphImpl implements Graph {
  ...
  addEdge(edgeConfig: Omit<EdgeType, 'id'> & { id?: string }): EdgeImpl {
    ...
    if (!sourceNode.outputs.has(edgeConfig.sourcePortId)) {
      throw new Error(
        `Source port ${edgeConfig.sourcePortId} not found on node ${edgeConfig.sourceNodeId}`,
      );
    }
    if (!targetNode.inputs.has(edgeConfig.targetPortId)) {
      throw new Error(
        `Target port ${edgeConfig.targetPortId} not found on node ${edgeConfig.targetNodeId}`,
      );
    }
  ...
  async execute(
    ...
    options?: { silent?: boolean; logLevel?: LogLevel },
  ): Promise<GraphState> {
    if (options?.logLevel) {
